"""
spec_cycle_logger.py
────────────────────
Per-cycle timing and data logger for speculative decoding experiments.
Covers EAGLE3, DFlash, and future draft strategies — any method that follows
the draft→verify→(extend) cycle structure.


Environment variables controlling behaviour:
  SPEC_CYCLE_LOG_ENABLE    - "1" to enable all logging (default: "1")
  SPEC_CYCLE_LOG_PATH      - absolute directory where JSONL files are written.
                                 The benchmark script sets this per config/dataset/rep/turn.
                                 Default: <repo_root>/outputs/spec_cycle_logs
                                 (auto-detected relative to this file; works on both
                                  DeltaWW /mnt/zhiqi/... and MIRLab /home/r13922176/...)
  SPEC_ATTN_MS_LOG_ENABLE     - "1" to additionally instrument the FlashInfer
                                 TARGET_VERIFY attention call with torch.cuda.Event
                                 and emit `attn_ms` in timing.jsonl. Default: "0".
                                 Subordinate to SPEC_CYCLE_LOG_ENABLE — if that
                                 is 0, attn_ms logging is also disabled.
  SPEC_TARGET_LOGITS_LOG_ENABLE - "1" to log target model log-probs and top-N
  ...
  SPEC_TARGET_LOGITS_TOPK     - How many top tokens to log per accepted depth
                                 position (default: 5).  Must be a positive integer.

When SPEC_CYCLE_LOG_ENABLE != "1", every function in this module
...
_DEFAULT_LOG_PATH = str(
    Path(__file__).resolve().parents[5] / "outputs" / "spec_cycle_logs"
)

ENABLED: bool = os.environ.get("SPEC_CYCLE_LOG_ENABLE", "1") == "1"
ATTN_MS_ENABLED: bool = os.environ.get("SPEC_ATTN_MS_LOG_ENABLE", "0") == "1"
...
# --log-target-logits flag (sets SPEC_TARGET_LOGITS_LOG_ENABLE=1).
# On by default — pass --no-log-target-logits or set env var to "0" to suppress.
TARGET_LOGITS_ENABLED: bool = os.environ.get("SPEC_TARGET_LOGITS_LOG_ENABLE", "1") == "1"
