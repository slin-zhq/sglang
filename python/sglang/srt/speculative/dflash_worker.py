...
class DFlashWorker:
    ...
    def forward_batch_generation(
        ...
        # === INSTRUMENTATION: DFlash per-cycle timing ===
        ...
        # Log per-cycle timings using the same logger as EAGLE3.
        # DFlash has no separate "extend" phase, so extend_ms=0.
        # accept_length = num_correct_drafts + 1 (bonus token always produced).
        if _exp_logger.ENABLED:
            _t_cycle_end = _exp_logger._sync_and_time()
            _batch_size = len(num_correct_drafts_per_req_cpu)
            _accept_length = (
                num_correct_drafts_per_req_cpu[0] + 1 if _batch_size == 1
                else sum(n + 1 for n in num_correct_drafts_per_req_cpu) / _batch_size
            )
